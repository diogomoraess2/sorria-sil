...
# --- CONFIGURAÇÃO DO SELETOR E TÍTULO ---
...
with st.container():
    with st.popover(f"Mês: {nome_aba_trabalho}", use_container_width=False):
        novo_mes_num = st.selectbox(
            "Selecione o mês:", 
            list(MESES_PT.keys()), 
...
st.markdown(f"""
    <div style="margin-top: -35px; margin-left: 55px; pointer-events: none;">
...
with tab_novo:  
    ...
    with st.form("form_registro", clear_on_submit=True):  
        ...
        if btn_salvar and validacao_domingo:  
            ...
            if sucesso:
                st.success(f"🎉 Registro de R$ {val_total_dia:.2f} adicionado!")
                st.rerun()
...
with tab_dados:  
    ...
    if not df_mes.empty:  
        ...
        df_exibir = df_exibir.rename(columns={'Total': 'Total (R$)', 'Dinheiro': 'Din (R$)', 'Pix': 'Pix (R$)', 'Próximo mês': 'Próx (R$)', 'Uber': 'Uber (R$)'})  
...
with tab_graficos:  
    ...
    if not df_mes.empty:  
        ...
        fig_linha = px.line(df_mes_ordenado, x='Data', y='Total', markers=True)  
